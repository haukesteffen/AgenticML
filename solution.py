...
HYPOTHESIS = "cuML KNN k=15 distance, target-encoded categoricals + numerics"
...
def _target_encode(X_tr, y_enc, X_vl, n_classes, smoothing=30):
    ...
    for ci, col in enumerate(_CAT_COLS):
        ...
        cats_tr = X_tr[col].fillna("__NaN__").astype(str)
        cats_vl = X_vl[col].fillna("__NaN__").astype(str)
        cat_map = {}
        for cat in cats_tr.unique():
            mask = cats_tr == cat
            n = mask.sum()
